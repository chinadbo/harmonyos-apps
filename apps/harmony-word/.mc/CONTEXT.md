...
## Import 风格
- 统一使用 @ohos.* 风格（如 `import rdb from '@ohos.data.rdb'`）
- 不使用 @kit.* 风格
...
## 本项目特定规范（HarmonyWord）
- RDB 使用 @ohos.data.rdb，初始化在 EntryAbility.onCreate
- Canvas 笔顺动画用 CanvasRenderingContext2D，注意 settings 参数
- 通知用 @ohos.notification，权限只需 PUBLISH_AGENT_REMINDER
- 词库数据以 const 数组形式内嵌到 DatabaseHelper.ets（无需网络）
- 拼音索引栏用 AlphabetIndexer 组件（@ohos 原生）
